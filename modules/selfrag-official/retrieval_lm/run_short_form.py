...
def postprocess_answer_option_conditioned(answer):
    ...
    # If answer becomes empty after removing control tokens, return a placeholder
    if not answer.strip():
        print(f"WARNING: Empty answer after postprocessing. Original: '{original_answer}'")
...
def call_model_rerank_w_scores_batch(prompt, evidences, model, max_new_tokens=15,
    ...
    if mode == "always_retrieve":
        ...
        if threshold is not None:
            ...
            for tok, id in ret_tokens.items():
                if not pred_log_probs or id not in pred_log_probs[0]:
                    score_dict[tok] = -100
                else:
    ...
    if do_retrieve is True:
        ...
        for p_idx, pred in enumerate(preds):
            ...
            if p_idx == 0:
                print(f"DEBUG: Generated text for evidence 0: '{pred_text}'")
            ...
            for tok, id in rel_tokens.items():
                prob = pred_log_probs[0][id] if pred_log_probs and id in pred_log_probs[0] else -100
            ...
            if grd_tokens is not None:
                ...
                if len(groundness_token_appear_indices) > 0:
                    ...
                    for token, token_id in grd_tokens.items():
                        prob = pred_log_probs[idx][token_id] if idx < len(pred_log_probs) and token_id in pred_log_probs[idx] else -100
        ...
        print(f"DEBUG: Generated text (no retrieval): '{pred}'")
