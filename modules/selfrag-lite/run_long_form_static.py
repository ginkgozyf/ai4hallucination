...
该脚本实现仓库中用于运行按需检索生成实验的主循环（SELF-RAG）。
支持多种任务（例如 `factscore`, `asqa`, `eli5`）并提供控制检索行为、
grounding token 和树搜索参数的选项。

在脚本中加入了轻量级的计时埋点（总耗时与平均每样本耗时），
以便后续对比不同策略（例如 SelfCheckGPT 与 SELF-RAG）时统计时间开销。
"""
...
def main():
    ...
    parser.add_argument('--download_dir', type=str, help="specify vllm model download dir",
                        default="/openbayes/home/model_cache")
    ...
    if args.world_size is not None:
        model = LLM(model=args.model_name, download_dir=args.download_dir,
                    dtype=args.dtype, tensor_parallel_size=args.world_size,  max_logprobs=50000)
    ...
    if args.task is not None and args.task == "factscore":
        ...
        for idx, item in enumerate(input_data):
            ...
            # 每样本输出与计时（中文）
            print(f"样本 {idx+1}/{len(input_data)} 完成，耗时 {sample_elapsed:.2f}s，输出预览：{postprocessed_result[:200]}")
        ...
        avg = elapsed / max(len(input_data), 1)
        print(f"factscore 已完成：总耗时 {elapsed:.2f}s，平均每样本 {avg:.2f}s")
        ...
        for instance_idx, item in enumerate(input_data):
            ...
            sample_elapsed = time.time() - instance_start
            ...
            item['selfrag_time'] = sample_elapsed
            item['self_rag_answer'] = item["output"]
            new_results["data"].append(item)

            # 每样本输出与计时（中文）
            preview = item.get("output", "")[:300]
            print(f"({instance_idx+1}/{len(input_data)})问题: {item['question']}\n")
            print(f"SelfRag回答: {preview}\n[用时: {sample_elapsed:.2f}秒]\n{'-'*40}\n")

            if instance_idx % 10 == 0:
                with open(args.output_file + "_tmp", 'w') as writer:
                    json.dump(new_results, writer)
        with open(args.output_file, 'w') as writer:
            json.dump(new_results, writer, indent=4)
        end_time = time.time()
        elapsed = end_time - start_time
        avg = elapsed / max(len(input_data), 1)
        print(f"QA/asqa/eli5 已完成：总耗时 {elapsed:.2f}s，平均每样本 {avg:.2f}s")
    else:
...
python run_long_form_static.py \
  ...
  --output_file output1 --max_depth 7 --mode always_retrieve 
"""