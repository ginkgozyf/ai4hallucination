...
from llm_hallucination_evaluate.solver.self_rag_utils import TASK_INST, PROMPT_DICT, load_special_tokens, load_jsonlines, postprocess, fix_spacing
...
args = {
    ...
    'output_file': './self_rag_experiment_file.json',
    'task': 'asqa',
    'device': 'cuda',
    'max_new_tokens': 300,
    'tokenizer_path': None,
    'download_dir': '/openbayes/home/model_cache',
    'ndocs': 5,
    'world_size': 1,
    'dtype': 'half',
    'threshold': 0.2,
...
def main():
    ...
    input_data = input_data[:2]  # 限制为前10个样本用于测试

    # 处理ASQA任务
    if args.task == "asqa":
        new_results = {"data": [], "args": [], "total_cost": 0.0, "azure_filter_fail": ""}
        ...
        for instance_idx, item in enumerate(input_data):
            ...
            if "splitted_sentences" not in intermediate:
                ...
                item["output"] = fix_spacing(final_output)
                item["output"] = item["output"].replace(".[Continue to Use Evidence]", " [1]. ")
        ...
        print(f"ASQA任务已完成：总耗时 {elapsed:.2f}s，平均每样本 {avg:.2f}s")
